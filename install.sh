...
install_linux () {
  ...
  if [ "${Distro}" = "Debian" ] || [ "${Distro}" = "Kali" ] || [ "${Distro}" = "Parrot" ] ; then
    ...
    sudo apt install git make wget curl python2 python3 python3-pip nano proot cowsay perl figlet libcaca-dev caca-utils libffi-dev libgmp-dev libyaml-dev toilet ruby zsh zsh-syntax-highlighting zsh-autosuggestions translate-shell -y
